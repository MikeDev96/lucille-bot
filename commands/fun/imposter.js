  ...
  run (msg) {
    ...
    axios.get("http://server.haribodev.uk/imposter/versions/update")
      .then(res => {
        ...
        var time = res.data.substring(11)
        ...
        const embed = {
          embed: {
            ...
            fields: [
              {
                name: "Next Update",
                value: res.data === "N/A" ? "Hmm looks like there's no release date as of yet, but stay tuned!" : `${moment(new Date(year, month - 1, day)).format("dddd Do MMMM YYYY")} at ${time}`,
              },
              {
                name: "Countdown",
                value: countDown,
